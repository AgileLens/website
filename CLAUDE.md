...
- **Use Tailwind theme tokens** — never write an arbitrary bracketed `var()` color lookup as a class, always `text-muted`, `text-pink`, etc. (Tailwind v4's content scanner treats any bracketed-arbitrary-value string it finds, even in this doc, as a real class candidate and will fail to build if the placeholder inside isn't valid CSS — say it in prose, don't paste the bracket syntax here.)
